...
| `fetchTrendWidgets(params)`               | `GET` | `/dashboard/trends?range=...`                  | Return shape `{ diseaseTrends, vaccinationProgress, complianceScores }`.                                                   |
...
| `fetchTalukBreakdown(districtSlug)`       | `GET` | `/disease/districts/{district}/taluks`         | Wire this when backend exposes taluk drill-down (currently unused).                                                        |
